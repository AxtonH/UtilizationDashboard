...
- **Phase 5 — 5a-5e DONE** (commits 708eac9, 37b4c47, 0993dc0, d4ed444, 5e): sales-dashboard.js (4,157 lines) → `static/js/sales/` package: main.js 2,070 + period-utils.js 576 (pure helpers) + filter-state.js 292 (owns currentFilterState behind get/set accessors) + charts.js 762 (shared chartInstances object; canvas element ids left untouched in strings) + tables.js 585 (lists/tables + localized sort state; currentMonth read via provider hook). **Remaining 5f (optional):** fetchSalesData + load-state (`salesDataCache`, `currentMonth`, `isLoading`, `salesDataLoadingPromise`, `filtersReady` — all reassigned inside fetchSalesData; would need a load-state module with accessors like filter-state). main.js keeps: KPI updater, reapplyFilters, subscription stats card, external-hours renderers, overlay/clear/verify lifecycle, fetch orchestrator, auth flow, event wiring. CAVEAT: sales-tab authenticated behavior is not covered by the headless harness (auth-gated) — manually click through the Sales tab (charts, filters, tables, month change) before deploying Phase 5 work.
- **Phase 6 — NOT STARTED.**
