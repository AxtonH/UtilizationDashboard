...
- **Phase 5 — 5a+5b DONE** (commits 708eac9, 37b4c47): sales-dashboard.js → ES module `static/js/sales/main.js` (readyState-guarded init); `period-utils.js` extracted (553 lines, 19 pure helpers). main.js now 3,634 lines. **Remaining 5c+:** per the dependency analysis, next seams are (1) filter-state module owning `currentFilterState` (reassigned in reapplyFilters, the salesFiltersChanged handler, and fetchSalesData — needs get/set accessors) plus the seven `*ForCurrentFilters` wrappers; (2) charts module — the four chart `let` handles are reassigned by their own update fns AND `clearSalesUI` (old L2911-2923), so clearSalesUI must call an exported destroyAllCharts(), and handleSalesTabActivation's resize block (old L4050-4053) an exported resizeAllCharts(); Chart.register(ChartDataLabels) one-time call moves with them; (3) per-table modules (project table, subscriptions table — each with its own localized sort/cache `let`s); (4) fetchSalesData + load-state (`salesDataCache`, `currentMonth`, `isLoading`, `salesDataLoadingPromise`, `filtersReady` all reassigned inside it). No sloppy-mode hazards exist (verified). Old→new line shift in main.js: pre-split line N ≈ N+2 minus extracted ranges; safer to re-locate by content anchors.
- **Phase 6 — NOT STARTED.**
